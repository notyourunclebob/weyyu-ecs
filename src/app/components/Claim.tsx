...
import { Claim, Category } from "@/tools/claim.model";
...
export function ClaimComp({ claim }: { claim: Claim }) {
  ...
  const onApprove = async () => {
    const result = await sendJSONData(
      `/api/claim/updateStatus/${claim._id}`,
            ...
            <div className="space-y-6 flex-1">
              ...
              {claim.category.toString() === "Medical" && (
                <div className="text-white flex flex-row pl-2">
                  <input
                    type="checkbox"
                    id="medCheckbox"
                    checked={claim.category.faceHugger}
                ...
                <div className="flex flex-col gap-2">
                  <div className="flex flex-col">
                    ...
                    <input
                      ...
                      name="startLocation"
                      className="w-full px-4 py-3 bg-yutaniGrey border-2 border-yutaniGrey rounded text-black placeholder-yutaniGrey focus:outline-none focus:border-yutaniYellow"
                      value={claim.category.locationStart}
                      disabled
                    />
                  </div>
                  <div className="flex flex-col">
                    ...
                    <input
                      ...
                      name="endLocation"
                      className="w-full px-4 py-3 bg-yutaniGrey border-2 border-yutaniGrey rounded text-black placeholder-yutaniGrey focus:outline-none focus:border-yutaniYellow"
                      value={claim.category.locationEnd}
                      disabled
                    />
                  </div>
                  <div className="flex flex-col">
              ...
              {session?.user?.admin && claim.status === "pending" && (
                <div>
                  <label className="block text-yutaniGrey text-sm mb-2 font-light">
                    Comment:{" "}
                    <span className="text-yutaniGrey opacity-50">
                      (optional)
                    </span>
                  </label>
                  <textarea
                  ...
                  />
                  <p
                    className={`text-xs mt-1 text-right ${comment.length >= 255 ? "text-red-400" : "text-yutaniGrey opacity-50"}`}
                  >
                    {comment.length}/255
                  </p>
                </div>
